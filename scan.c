...
/* getNextChar fetches the next non-blank character
  ...
  exhausted 
  该函数将一个256-字符缓冲区内部的lineBuf中的字符取到扫描程序中
...
static int getNextChar(void)
{
  ...
  {
    lineno++;
    if (fgets(lineBuf, BUFLEN - 1, source))//fets从指定流source读取一行并存入lineBuf指向的字符串内
    {
      if (EchoSource)
...
/* lookup an identifier to see if it is a reserved word 检查标识符是否为保留字*/
/* uses linear search 表reservedWords和该函数完成由getToken的主要循环识别的标识符知乎的保留字的查找*/
...
{ /* index for storing into tokenString 用于存储到tokenstring的索引*/
  ...
  while (state != S_DONE)//根据DFA扫描
  {
    ...
    if ((save) && (tokenStringIndex <= MAXTOKENLEN))//如果可填入且长度符号要求，则填入tokenStirng中
